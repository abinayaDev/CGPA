...
predictBtn.addEventListener("click", async () => {
    ...
    try {
        ...
        if (!token) {
            ...
            const response = await fetch(API_BASE_URL + `/api/predict/${rollNo}`, {
            headers: {
...
async function handleLockSemester(event) {
    ...
    try {
        ...
        if (!token) {
            ...
            const response = await fetch(API_BASE_URL + "/api/lock_semester", {
            method: "POST",
            headers: {
    ...
    btnFilterExport.addEventListener("click", async () => {
        ...
        try {
            ...
            if (!token) {
                ...
                const response = await fetch(API_BASE_URL + "/api/export_filtered", {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Bearer ${token}`
...
if (btnExportAll) {
    btnExportAll.addEventListener("click", async () => {
        ...
        try {
            ...
            const downloadUrl = API_BASE_URL + "/api/export_all";
            ...
            predictionMessage.innerHTML = `<div class='alert alert-danger'>Export failed: ${e.message}</div>`;
        }
    });
}
